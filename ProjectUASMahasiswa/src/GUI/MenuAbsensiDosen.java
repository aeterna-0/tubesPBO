...
public class MenuAbsensiDosen extends JFrame {
    ...
    public MenuAbsensiDosen(Connection conn, String nidn) {
        ...
        btnKembali.setFont(new Font("Segoe UI", Font.BOLD, 18));
        btnKembali.setBackground(new Color(220, 53, 69)); // Warna Merah
        btnKembali.setForeground(Color.WHITE);
        btnKembali.setPreferredSize(new Dimension(150, 46)); // Sedikit lebih kecil dari Simpan
        ...
        // Masukkan tombol-tombol
        bottom.add(btnSave);            // Tombol Simpan (LAMA)
        bottom.add(Box.createHorizontalStrut(20)); // Jarak
        bottom.add(btnKembali);         // Tombol Kembali (BARU)
    ...
    private void simpanAbsensi() {
        ...
        // 1. Simpan Detail Absensi (Hadir/Izin/Sakit/Alpha)
        String sqlInsert = """
        ...
        // 2. Update Total Kehadiran di Tabel KRS (Hanya menghitung yang 'Hadir')
        String sqlUpdateKRS = """
                UPDATE krs k
                SET k.absensi = (
                    SELECT COUNT(*) 
        ...
        try {
            conn.setAutoCommit(false); // Biar aman, gunakan transaksi
            ...
            // --- EKSEKUSI UPDATE REKAP KRS ---
            try (PreparedStatement psUpdate = conn.prepareStatement(sqlUpdateKRS)) {
                psUpdate.setString(1, kodeMk);
            ...
            JOptionPane.showMessageDialog(this, "Absensi berhasil disimpan & KRS diperbarui!");

        } catch (SQLException ex) {
            try { conn.rollback(); } catch (SQLException e) {} // Batalkan jika error
